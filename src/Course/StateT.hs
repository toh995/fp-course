...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) stateTf stateTa =
    StateT $
      \s ->
        runStateT stateTf s >>= \(f, s') ->
        runStateT stateTa s' >>= \(a, s'') ->
        pure (f a, s'')
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' state' = runExactlyOne . runStateT state'
...
execT ::
  ...
  -> s
  -> k s
execT stateTa = (<$>) snd . runStateT stateTa
...
exec' ::
  ...
  -> s
  -> s
exec' stateA' = snd . runState' stateA'
...
evalT ::
  ...
  -> s
  -> k a
evalT stateTa = (<$>) fst . runStateT stateTa
...
eval' ::
  ...
  -> s
  -> a
eval' stateA' = fst . runState' stateA'
  ...
  where
    p a =
      getT >>= \s ->
        if S.member a s
        then pure False
        else pure True <* putT (S.insert a s)
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<$>) f (OptionalT optWrapper) =
    OptionalT $ (f <$>) <$> optWrapper
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<*>) (OptionalT kOptF) (OptionalT kOptA) =
    OptionalT $
      kOptF >>= \case
        Empty ->
          pure Empty
...
distinctG as =
  runOptionalT $ evalT (filtering p as) S.empty
  where
    p a
      | a > 100   = StateT $ \s ->
                      OptionalT (
                        Logger
                          (("aborting > 100: " ++ show' a) :. Nil)
                          Empty
                      )
      | even a    = StateT $ \s ->
                      OptionalT (
                        Logger
                          (("even number: " ++ show' a) :. Nil)
